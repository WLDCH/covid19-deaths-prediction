...
@task
def scrap_data(scrap_covid_indicator=True, scrap_covid_test=True):
    covid_indicator, covid_test = None, None

    if scrap_covid_indicator:
    ...
    return(covid_indicator, covid_test)


@task
def read_data(covid_indicator_path=None, covid_test_path=None):
    if covid_indicator_path is None:
        covid_indicator_df = scrap_data(scrap_covid_indicator=True, scrap_covid_test=False)

    if covid_test_path is None:
        covid_test_df = scrap_data(scrap_covid_indicator=False, scrap_covid_test=True)
