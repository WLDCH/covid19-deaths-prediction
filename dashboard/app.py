...
def plot_death_prediction(deaths_so_far, deaths_prediction):
    ...
    fig.add_trace(
        go.Scatter(
            x=deaths_prediction.index,
            y=deaths_prediction.values.reshape(-1),
            name="Deaths predictions",
            line_shape="linear",
        )
    )
    fig.add_trace(
        go.Scatter(
            x=deaths_so_far[-20:].time_index,
            y=deaths_so_far[-20:].values().reshape(-1),
            name="Past deaths",
            line_shape="linear",
        )
    )
...
if __name__ == "__main__":
    ...
    with st.sidebar.expander("Note on model retraining"):
        st.write(note)
    plot_spot = st.empty()
